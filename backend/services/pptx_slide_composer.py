...
def save_slide_md(md_path: str, drafted_text: str, output_dir: str | None = None) -> str:
    """LLM 이 뱉은 MD 를 `{앞3자}_sl_{ts}.md` 로 저장.
    output_dir 지정 시 그 폴더에, 없으면 입력과 같은 폴더에.
    """
    import time as _time
    p = Path(md_path)
    ts = _time.strftime("%y%m%d_%H%M%S")
    name = f"{short_stem(p.stem)}_sl_{ts}.md"
